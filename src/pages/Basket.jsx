...
import {clearBasket, removeBasketItem, minusBasketItem, plusBasketItem} from '../redux/actions/basket';
...
const Basket = () => {
  ...
  const pizzasToBasket = Object.keys(items).map(key => {
    return {
      itemsObj: items[key].items[0],
  ...
  const onMinusBasketItem = (index) => {
    dispatch(minusBasketItem(index));
  }

  const onPlusBasketItem = (index) => {
    dispatch(plusBasketItem(index));
  }
      ...
      ?
        <div className="cart">
          <div className="cart__top">
            ...
            <div className="cart__clear" onClick={onClearBasket}>
          ...
          <div className="content__items">  
            {
            pizzasToBasket.map((itemObj) => 
              <BasketItem 
                key={itemObj.itemsObj.id} 
                itemObj={itemObj} 
                onItemRemove={onItemRemove}
                onMinusBasketItem={onMinusBasketItem} 
                onPlusBasketItem={onPlusBasketItem} 
              />
            )}
          </div>
